...
if TYPE_CHECKING:
    ...
    type JSONSerializable = str

    class UserStateDataMapping(TypedDict, total=False):
        first_message_id: int  # points on the first dialogue message id
        message_queue: list[JSONSerializable]  # Allow to get last callback to make popup response.
        callback_called_once: bool  # Allow to check if we already answer on the message.
        action: SettingsAction
        callback: JSONSerializable  # Allow to get last message to recreate start dialogue from the beggining.
